 ...
 * RegistrationFormSP
 *
 * スマートフォン用レシピ登録フォームコンポーネント
 * - レシピの登録・編集機能
 * - 材料選択・管理機能
 * - 栄養素計算機能
 * - AI説明文生成機能
 */
export const RegistrationFormSP = ({
  isAdmin = false,
  initialRecipe,
}: RecipeFormProps) => {
  const {
              ...
              {formData.ingredients.map((ingredient) => {
                ...
                const handleQuantityChange = (delta: number) => {
                  ...
                  const isQuantityTypeWithStep1 =
                    selectedUnit.type === "quantity" && selectedUnit.step === 1;
                    ...
                    <div className={styles.select_ingredient_block__contents}>
                      ...
                      <div className={styles.select_ingredient_block__controls}>
                        <button
                          ...
                          className={`${
                            styles.select_ingredient_block__button
                          } ${
                            styles["select_ingredient_block__button--plus"]
                          } ${
                            PRESENCE_UNITS.includes(
                              ingredient.unit as (typeof PRESENCE_UNITS)[number]
                            )
                              ? styles[
                                  "select_ingredient_block__button--disabled"
                                ]
                              : ""
                          }`}
                        ...
                        >
                          {PRESENCE_UNITS.includes(
                            ...
                            : ingredient.unit === "大さじ" ||
                              ingredient.unit === "小さじ"
                            ? `${ingredient.unit}${formatQuantity(
                                ingredient.quantity
                              )}`
